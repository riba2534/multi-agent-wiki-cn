...
| 需要质量审查 | Generator-Critic / Refinement Loop | 单智能体自评分 | 自评分常常复现相同错误 |
...
| 跨智能体/跨厂商 | A2A / ACP | 私有 RPC | 更适合互操作性 |
| IDE 到编码智能体 | Agent Client Protocol | 临时 HTTP API | 更接近编码智能体标准接入方式 |
...
1. **一开始就上 Group Chat。** 看起来像多智能体，但很少收敛。
2. **长任务没有状态机。** 半途任务无法恢复，也无法调试。
3. **所有智能体共享一个上下文。** 污染严重，权限边界消失。
4. **代码生成没有验证器。** 智能体会自信地生成错误的 diff。
