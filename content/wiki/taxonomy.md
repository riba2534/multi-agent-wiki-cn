---
title: 分类法
description: 多 Agent 模式的工程维度分类法
---
...
当任务规模超出单轮对话、甚至超出少量子智能体的稳定范围时，编排逻辑应当从「每轮临场判断」固化为**显式代码**。

| 类型 | 编排逻辑在哪里 | 代表模式 |
|---|---|---|
| 对话式编排 | 隐含在主智能体每轮推理中 | 监督者、子智能体即工具 |
| 团队协作 | 共享任务列表 + 互发消息 + lead | Agent Teams（参见监督者 / 管理者） |
| 代码编排 | 一段可执行脚本持有计划、循环、扇出、验证与收敛 | [动态工作流 / 代码编排子智能体](patterns/dynamic-workflow-code-orchestration) |

代码编排把计划变成一等产物（脚本），中间状态留在脚本变量、可恢复、可保存复用，规模可达数百个子智能体。完整工程实践见[工作流板块](workflows)。
