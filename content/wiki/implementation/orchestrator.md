---
title: 编排器实现指南
description: 多 Agent 平台的调度器、路由器和状态机
---

# 编排器实现指南

编排器是多 Agent 平台的核心。它**不是**"一个会写优质提示词的 Agent"——它是**工作流引擎 + 策略引擎 + Agent 调度器 + 追踪事件发射器**的组合。
...
- 编排器自己包办一切——退化为单一 Agent。
- 没有检查点机制——任务无法恢复。
- LLM 路由缺少策略护栏——Agent 调用不该用的工具。
- 子 Agent 输出缺少模式约束——下游节点无法解析。