---
title: 可观测性与事件模型
description: 多 Agent 平台的追踪、事件和指标设计。
---

# 可观测性与事件模型

缺乏追踪能力，多 Agent 平台基本上难以维护。你需要记录的不仅是最终答案——**每一次路由决策、消息、工具调用、交接、状态变更、审批、失败和重试**都需要记录。
...
| `agent.message.created` | Agent 生成消息 |
| `agent.task.assigned` | 任务已分配给 Agent |
